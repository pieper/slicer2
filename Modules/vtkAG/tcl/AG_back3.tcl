...
#def TransformScale(Target,Source,scale):   
   ...
   div SetInput1  [Volume($AG(InputVolTarget),vol) GetOutput]
   div Update
   Volume($AG(InputVolTarget),vol) DeepCopy [div GetOutput]

   vtkImageMathematics div2
   div2 SetOperationToMultiplyByK
   div2 SetConstantK $AG(Scale)
   div SetInput1  [Volume($AG(InputVolSource),vol) GetOutput]
   div Update
   Volume($AG(InputVolSource),vol) DeepCopy [div GetOutput]
   return 1
}
...
proc RunAG {} {
  ...
  if {$AG(Linear)} {
      ...
      GCR.SetSource(Source) [ Volume($AG(InputVolSource),vol) GetOutput]
