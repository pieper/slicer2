...
ELSE (USE_VTK_FILE)
  SET (VTKMRPROSTATECARE_CAN_BUILD 0)
ENDIF (USE_VTK_FILE)
...
SET (LIBRARY_OUTPUT_PATH ${VTKMRPROSTATECARE_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKMRPROSTATECARE_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKMRPROSTATECARE_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKMRPROSTATECARE built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKMRPROSTATECARE_WRAP_TCL 
         "Wrap classes into the TCL interpreted language." 
         ON)

  IF (VTKMRPROSTATECARE_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKMRPROSTATECARE_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKMRPROSTATECARE_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKMRPROSTATECARE_WRAP_TCL)
    MESSAGE("Warning. VTKMRPROSTATECARE_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKMRPROSTATECARE_WRAP_TCL to OFF.")
    SET (VTKMRPROSTATECARE_WRAP_TCL OFF)
  ENDIF (VTKMRPROSTATECARE_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKMRPROSTATECARE_WRAP_PYTHON 
         "Wrap classes into the Python interpreted language." 
         ON)

  IF (VTKMRPROSTATECARE_WRAP_PYTHON)
  ...
  ENDIF (VTKMRPROSTATECARE_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKMRPROSTATECARE_WRAP_PYTHON)
    MESSAGE("Warning. VTKMRPROSTATECARE_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKMRPROSTATECARE_WRAP_PYTHON to OFF.")
    SET (VTKMRPROSTATECARE_WRAP_PYTHON OFF)
  ENDIF (VTKMRPROSTATECARE_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKMRPROSTATECARE_WRAP_JAVA 
         "Wrap classes into the Java interpreted language." 
         ON)

  IF (VTKMRPROSTATECARE_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKMRPROSTATECARE_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKMRPROSTATECARE_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKMRPROSTATECARE_WRAP_JAVA)
    MESSAGE("Warning. VTKMRPROSTATECARE_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKMRPROSTATECARE_WRAP_JAVA to OFF.")
    SET (VTKMRPROSTATECARE_WRAP_JAVA OFF)
  ENDIF (VTKMRPROSTATECARE_WRAP_JAVA)
