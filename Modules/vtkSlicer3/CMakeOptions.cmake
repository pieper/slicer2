...
SET (LIBRARY_OUTPUT_PATH ${VTKSLICER3_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKSLICER3_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKSLICER3_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKSLICER3 built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKSLICER3_WRAP_TCL 
         "Wrap classes into the TCL interpreted language." 
         ON)

  IF (VTKSLICER3_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKSLICER3_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKSLICER3_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKSLICER3_WRAP_TCL)
    MESSAGE("Warning. VTKSLICER3_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKSLICER3_WRAP_TCL to OFF.")
    SET (VTKSLICER3_WRAP_TCL OFF)
  ENDIF (VTKSLICER3_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKSLICER3_WRAP_PYTHON 
         "Wrap classes into the Python interpreted language." 
         ON)

  IF (VTKSLICER3_WRAP_PYTHON)
  ...
  ENDIF (VTKSLICER3_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKSLICER3_WRAP_PYTHON)
    MESSAGE("Warning. VTKSLICER3_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKSLICER3_WRAP_PYTHON to OFF.")
    SET (VTKSLICER3_WRAP_PYTHON OFF)
  ENDIF (VTKSLICER3_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKSLICER3_WRAP_JAVA 
         "Wrap classes into the Java interpreted language." 
         ON)

  IF (VTKSLICER3_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKSLICER3_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKSLICER3_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKSLICER3_WRAP_JAVA)
    MESSAGE("Warning. VTKSLICER3_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKSLICER3_WRAP_JAVA to OFF.")
    SET (VTKSLICER3_WRAP_JAVA OFF)
  ENDIF (VTKSLICER3_WRAP_JAVA)
