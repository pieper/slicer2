...
#ifndef _EMLOCALREGISTRATION_H_INCLUDED
#define _EMLOCALREGISTRATION_H_INCLUDED 1
...
} EMLocalRegistration_MultiThreadedParameters;

class EMLocalRegistration_ROI {
  public:
  char *MAP;
  int MinCoord[3];
  int MaxCoord[3];
  ...
  EMLocalRegistration_ROI() { this->MAP = NULL; memset(MaxCoord,0,sizeof(int)*3); memset(MinCoord,0,sizeof(int)*3);ClassOutside = -1;}
  ~EMLocalRegistration_ROI() {if (this->MAP) {delete[] this->MAP; this->MAP = NULL;}}
}; 

// For what ever reason the if you use a template class the functions have to be in the same file as the class definition
class VTK_EMLOCALSEGMENT_EXPORT EMLocalRegistration { 
public:
  // Core Function 
  float RegistrationCostFunction(float* parameters);
  void  StartRegistration(float* Parameters, float &Cost);
  // Core Function 
  ...
  // Setting Parameters 
  void SetROI_MinZ(int init){this->ROI_MinZ = init;}
  void SetROI_MinY(int init){this->ROI_MinY = init;}
  void SetROI_MinX(int init){this->ROI_MinX = init;}
  void SetROI_MaxZ(int init){this->ROI_MaxZ = init;}
  void SetROI_MaxY(int init){this->ROI_MaxY = init;}
  void SetROI_MaxX(int init){this->ROI_MaxX = init;}
  ...
  int GetBoundary_OffsetY(){return this->Boundary_OffsetY;}
  int GetBoundary_OffsetZ(){return this->Boundary_OffsetZ;}
  void SetBoundary_OffsetY(int init){this->Boundary_OffsetY = init;}
  void SetBoundary_OffsetZ(int init){this->Boundary_OffsetZ = init;}
  ...
  // Utility Function 
  void SetROI_Weight(EMLocalRegistration_ROI* init) {this->ROI_Weight = init;}
  void SetROI_ProbData(EMLocalRegistration_ROI* init) {this->ROI_ProbData = init;}
  ...
  void DefineRegistrationParametersForThreadedCostFunction(int ROI_MinX, int ROI_MinY, int ROI_MinZ, int ROI_MaxX, int ROI_MaxY, int ROI_MaxZ);
  ...
  void SetClassToAtlasRotationMatrix(float** init) {this->ClassToAtlasRotationMatrix = init;}
  void SetClassToAtlasTranslationVector(float** init) {this->ClassToAtlasTranslationVector = init;} 
  ...
  // Default functions
  EMLocalRegistration() {this->InitializeParameters();}
  ~EMLocalRegistration();

  // =====================
  int GetNumberOfThreads() { return this->NumberOfThreads;} 
  EMLocalRegistration_MultiThreadedParameters* GetMultiThreadedParameters() {return MultiThreadedParameters;}
  ...
  float** GetClassToAtlasRotationMatrix()         {return this->ClassToAtlasRotationMatrix;}
  float** GetClassToAtlasTranslationVector()      {return this->ClassToAtlasTranslationVector;}
  ...
  EMLocalRegistration_ROI* GetROI_Weight() {return this->ROI_Weight;}
  EMLocalRegistration_ROI* GetROI_ProbData() {return this->ROI_ProbData;}
  ...
  double GetMinCost() {return this->MinWeightAtlasCost + this->MinGaussianCost;} 
  double GetMinWeightAtlasCost() {return this->MinWeightAtlasCost;} 
  double GetMinGaussianCost() {return this->MinGaussianCost;} 

  void ResetMinWeightAtlasCost() {this->MinWeightAtlasCost = EMLOCALREGISTRATION_MAX_PENALITY; }
  void ResetMinCost() {this->ResetMinWeightAtlasCost();this->MinGaussianCost =0;}  
...
private:
  // Utility Functions
  void ScaleRotationValues(float *FinalParameters);
  ...
  EMLocalRegistration_ROI* ROI_Weight;
  EMLocalRegistration_ROI* ROI_ProbData;
  ...
  int NumberOfThreads;
  EMLocalRegistration_MultiThreadedParameters* MultiThreadedParameters;
...
template <class T> 
inline void EMLocalRegistration_DefineROI_ProbDataValues(EMLocalRegistration* self, T** ProbDataPtr) {
  ...
  EMLocalRegistration_ROI* ROI_ProbData = self->GetROI_ProbData();
  assert(ROI_ProbData);
