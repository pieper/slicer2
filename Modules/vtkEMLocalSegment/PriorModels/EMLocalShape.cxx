...
#include "assert.h"
  ...
  // Foreqard decleration
template  <class Tin> 
inline void EMLocalShape_CalculateShapeCostFunction(EMLocalShape *Shape, Tin **ProbDataPtrOrig, int *VoxelStart, int NumberOfVoxels, int DataJump, 
                                                      int *PCAMeanShapeJump, int **PCAEigenVectorsJump, int *ProbDataJump, float &result);

//----------------------------------------------------------------------------------
// Utility functions
//----------------------------------------------------------------------------------
template  <class Tin> void EMLocalShape_AssignProbDataPtr(EMLocalShape *self, Tin  **ProbDataPtr,int DataJump, int ClassIndex) {
...
// This function is called by this->Threader->SingleMethodExecute(). Also look at EMLocalShape::EMLocalShape for definition of this->Threader
VTK_THREAD_RETURN_TYPE EMLocalShape_ShapeCostFunctionMultiThreaded_Function(void *arg) {
  int CurrentThread = ((ThreadInfoStruct*)(arg))->ThreadID;
  EMLocalShape* shape = (EMLocalShape*) (((ThreadInfoStruct*)(arg))->UserData);
  
  // ID is to high for our setup 
  assert(CurrentThread < shape->GetNumOfThreads()); 
  EMLocalShape_MultiThreadedParameters* ThreadedParameters = &(shape->GetMultiThreadedParameters()[CurrentThread]);
  
  ThreadedParameters->Result = 0.0;
  switch (shape->GetProbDataType()) {                  
    vtkTemplateMacro(EMLocalShape_CalculateShapeCostFunction(shape, (VTK_TT**) shape->GetProbDataPtr(), ThreadedParameters->VoxelStart, 
    ...
    default :
     cout << "Warning: EMLocalShape_ShapeCostFunctionMultiThreaded_Function : unknown data type " << shape->GetProbDataType() << endl;
  }
...
inline void EMLocalShape_CalculateShapeCostFunction(EMLocalShape *Shape, Tin **ProbDataPtrOrig, int *VoxelStart, int NumberOfVoxels, int DataJump, 
  ...
  // cout << "Start vtkImageEMLocalSegmenter_CalculateShapeCostFunction " << endl;
  // --------------------------------------
  // Registration Parameters 
  // --------------------------------------
  
  float* ShapePara = Shape->GetPCAPara();
...
EMLocalShape::EMLocalShape(EMLocal_Hierarchical_Class_Parameters* initEMHierarchyParameters, int  *initPCANumberOfEigenModes, int DisableMultiThreading) {
  ...
  if (DisableMultiThreading) this->NumOfThreads = 1;
  else this->NumOfThreads = vtkMultiThreader::GetGlobalDefaultNumberOfThreads(); 
  //cout << "====================================== Debug "<< this->NumOfThreads << endl;

  this->Threader = vtkMultiThreader::New();
  this->Threader->SetNumberOfThreads(this->NumOfThreads);
  this->Threader->SetSingleMethod(EMLocalShape_ShapeCostFunctionMultiThreaded_Function, ((void*) this));

  this->MultiThreadedParameters = new EMLocalShape_MultiThreadedParameters[this->NumOfThreads];
...
//----------------------------------------------------------------------------
// Set/Get Functions
void EMLocalShape::SpatialCostFunctionOn() {
     assert(this->NumberOfVoxelsInImage < 0);
     if (this->SpatialCostFunction) delete[] this->SpatialCostFunction;
     SpatialCostFunction = new float[this->NumberOfVoxelsInImage];
}
 
void EMLocalShape::SpatialCostFunctionOff() {
    if (this->SpatialCostFunction) delete[] this->SpatialCostFunction;
    this->SpatialCostFunction = NULL;
} 


void EMLocalShape::InitRegSpecPara(int initRegistrationType, int LengthX, int LengthY, int LengthZ)  {
...
void EMLocalShape::DefinePCADataPtr(int inPCAMinX, int inPCAMinY, int inPCAMinZ, int inBoundaryMaxX, int inBoundaryMaxY, int inDataIncY,  int inDataIncZ, int &outJump, int &outDataIncY,  int &outDataIncZ) {  
...
inline double EMLocalShape::Transfere_DistanceMap_Into_SpatialPrior(double distance, float variance, float boundary, float Min, float Max) {
...
float EMLocalShape::ShapeCostFunctionMultiThreaded(float *initPCAPara) {
  
  this->PCAPara = initPCAPara;
  if (this->SpatialCostFunction) memset(this->SpatialCostFunction, 0, sizeof(float)*this->NumberOfVoxelsInImage);
  // Start Execution
  ...
  for (int i = 0 ; i <  this->NumberOfTotalTypeCLASS; i++) {
    ...
    // this is defined as 
    PCAPenalty += PCAPara[ShapeIndex]*PCAPara[ShapeIndex]*this->PCAInverseEigenValues[i][k] ;
    ShapeIndex ++;
      }
  ...
  this->GaussianPenalty = PCAPenalty;
  this->ImagePenalty    = result;
...
void EMLocalShape::CalculateOptimalParameters(float **PCAShapeParameters, int PCAMaxX, int PCAMinX, int PCAMaxY, int PCAMinY, 
                                           ...
                                           int* initPCAMeanShapeIncY, int *initPCAMeanShapeIncZ, 
                                           float*** initPCAEigenVectorsPtr, int **initPCAEigenVectorsIncY, int** initPCAEigenVectorsIncZ, float &PCACost) {
  ...
  // cout << "Start vtkImageEMLocalSegmenter_ShapeInterface  " << endl;
   ...
   float *PCAParameters  = new float[this->PCATotalNumOfShapeParameters]; 
      ...
      if (initProbDataPtr[i]) { 
        ...
        this->DefinePCADataPtr(PCAMinX, PCAMinY, PCAMinZ, Boundary_LengthX, Boundary_LengthY, this->EMHierarchyParameters->ProbDataIncY[i], 
           ...
           switch (this->GetProbDataType()) {
             vtkTemplateMacro(EMLocalShape_AssignProbDataPtr(this, (VTK_TT**) initProbDataPtr,DataJump,i));

         default :
               cout << "EMLocalShape::CalculateOptimalParameters Unknown ScalarType " << this->GetProbDataType() << endl;
           return;
       }
   ...
   EMLocalShape_MultiThreadedParameters *MultiThreadedParameters = this->MultiThreadedParameters;  
   ...
   // Maximize
   if (1) {
     cout << "We start Powells Method now" << endl;
     ConjugateMethods::powell_minimize(this->PCATotalNumOfShapeParameters, PCAParameters, 1.0, ((void*) this), EMLocalShape_ShapeCostFunction, &PCACost, 
                                       NumOfFunctionEvaluations);
     cout << "Number of Evaluations :" << NumOfFunctionEvaluations << endl;
     cout << "End with Powells Method now" << endl;
   } 
   ...
   // cout << "End vtkImageEMLocalSegmenter_ShapeInterface  " << endl;
}  
