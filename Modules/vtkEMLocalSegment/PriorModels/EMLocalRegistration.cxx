...
/* Forward declaration */
float EMLocalRegistration_RegistrationCostFunction(void* selfFunc, float* parameters);

template <class T> void EMLocalRegistration_PrintVector(T* parameters, int Min,  int Max) {
  for (int i = Min; i <= Max ; i++)  cout << parameters[i] << " ";
  cout << endl;
}


void EMLocalRegistration_ROI::CreateMAP(int size)  {
  assert(size > 0);
  assert(!this->MAP);
...
void EMLocalRegistration::InitializeParameters() {
  // cout << "EMLocalRegistration::InitializeParameters initialize with all the ProbData stuff" << endl;
  ...
  ClassToAtlasRotationMatrix         = NULL;
  ClassToAtlasTranslationVector      = NULL; 
...
EMLocalRegistration::~EMLocalRegistration() {
    this->ClassInvCovariance_Delete(); 
    this->SpatialCostFunctionOff();
    this->MultiThreadDelete();
}

void EMLocalRegistration::SetBoundary(int MinX, int MinY, int MinZ, int MaxX, int MaxY, int MaxZ) {
...
void EMLocalRegistration::SetImage_Length(int LengthX, int LengthY, int LengthZ) 
 {
...
void EMLocalRegistration::SetDimensionOfParameter(int inNumberOfParameterSets, int inTwoDFlag, int inRigidFlag) {
...
void EMLocalRegistration::ClassInvCovariance_Define(classType* ClassListType, void** ClassList) {
...
void EMLocalRegistration::ClassInvCovariance_Print() {
     for (int i = this->GenerateBackgroundProbability; i < this->EMHierarchyParameters.NumClasses; i ++) {
       cout << "Class " << i << " " ; 
...
inline void EMLocalRegistration::DefineRegistrationParametersForThreadedCostFunction(int initROI_MinX, int initROI_MinY, int initROI_MinZ, int initROI_MaxX, int initROI_MaxY, int initROI_MaxZ) {
  ...
  if (initROI_MinX < Boundary_MinX ) this->ROI_MinX = Boundary_MinX;
  else this->ROI_MinX = initROI_MinX;

  if (initROI_MinY < Boundary_MinY) this->ROI_MinY = Boundary_MinY;
  else this->ROI_MinY = initROI_MinY;

  if (initROI_MinZ < Boundary_MinZ) this->ROI_MinZ = Boundary_MinZ;
  else this->ROI_MinZ = initROI_MinZ;


  if (initROI_MaxX > Boundary_MaxX) this->ROI_MaxX = Boundary_MaxX;
  else this->ROI_MaxX = initROI_MaxX;
  if (initROI_MaxY > Boundary_MaxY) this->ROI_MaxY = Boundary_MaxY;
  else this->ROI_MaxY = initROI_MaxY;
  if (initROI_MaxZ > Boundary_MaxZ) this->ROI_MaxZ = Boundary_MaxZ;
  else this->ROI_MaxZ = initROI_MaxZ;

  // --------------------------------------------------
  // Define Registration ROI Specific Parameters
  int ROI_LengthX           = this->ROI_MaxX -  this->ROI_MinX + 1;
  int ROI_LengthY           = this->ROI_MaxY -  this->ROI_MinY + 1;
  int ROI_LengthZ           = this->ROI_MaxZ -  this->ROI_MinZ + 1;
  ...
  int Image_VoxelOffset = this->ROI_MinZ*Image_LengthXY + this->ROI_MinY * Image_LengthX + this->ROI_MinX;    
  ...
  this->Boundary_OffsetZ     = (this->Boundary_LengthY - ROI_LengthY)*this->Boundary_LengthX;
  this->Boundary_OffsetY     = this->Boundary_LengthX - ROI_LengthX;
  int Boundary_VoxelOffset      = (this->ROI_MinX - Boundary_MinX) + (this->ROI_MinY - Boundary_MinY)* Boundary_LengthX + (this->ROI_MinZ - Boundary_MinZ)* Boundary_LengthXY;
...
void EMLocalRegistration::MultiThreadDelete() { 
...
void EMLocalRegistration::SpatialCostFunctionOff() { 
    if (this->SpatialCostFunction) delete[] this->SpatialCostFunction;
    this->SpatialCostFunction = NULL;
  }

void EMLocalRegistration::ClassInvCovariance_Delete() {    
...
VTK_THREAD_RETURN_TYPE EMLocalRegistration_CostFunction_Sum_WeightxProbability_Thread(void *arg) {
  int CurrentThread = ((ThreadInfoStruct*)(arg))->ThreadID;
  EMLocalRegistration* self = (EMLocalRegistration*) (((ThreadInfoStruct*)(arg))->UserData);

  assert(CurrentThread < self->GetNumberOfThreads()); 

  EMLocalRegistration_MultiThreadedParameters* ThreadedParameters = &(self->GetMultiThreadedParameters()[CurrentThread]);
...
void EMLocalRegistration::MultiThreadDefine(int DisableFlag) {
  this->MultiThreadDelete();
  if (DisableFlag) this->NumberOfThreads = 1;
  else this->NumberOfThreads = vtkMultiThreader::GetGlobalDefaultNumberOfThreads(); 
 
  //cout << "Registration Debug NumverOf Treads " << NumberOfThreads << endl;
  this->MultiThreadedParameters = new EMLocalRegistration_MultiThreadedParameters[this->NumberOfThreads];
  this->Threader = vtkMultiThreader::New();
  this->Threader->SetNumberOfThreads(NumberOfThreads);
  this->Threader->SetSingleMethod(EMLocalRegistration_CostFunction_Sum_WeightxProbability_Thread, this); 
}
...
inline void EMLocalRegistration::CostFunction_Sum_WeightxProbability(int Real_VoxelStart[3], int Boundary_VoxelOffset, int ROI_NumVoxels, double &result) {
  ...
  for (int voxel = 0; voxel < ROI_NumVoxels; voxel++) {
    ...
    EMLocalInterface_findCoordInTargetOfMatchingSourceCentreTarget(this->ClassToAtlasRotationMatrix[NumClassesMinusOne], this->ClassToAtlasTranslationVector[NumClassesMinusOne], 
    ...
    for (int h = NumClassesMinusOne; h > -1; h--) {
      ...
      if (h || !this->GenerateBackgroundProbability) {
        ...
        if (this->RegistrationType > EMSEGMENT_REGISTRATION_GLOBAL_ONLY) {
          EMLocalInterface_findCoordInTargetOfMatchingSourceCentreTarget(this->ClassToAtlasRotationMatrix[h], this->ClassToAtlasTranslationVector[h], x, y, z, 
    ...
    assert(z <= this->ROI_MaxZ);

    if (x > this->ROI_MaxX) {
      x = this->ROI_MinX; 
      Boundary_ROIVectorPtr += this->Boundary_OffsetY; 
      if (SpatialCostFunctionPtr) SpatialCostFunctionPtr += this->Boundary_OffsetY;
      if (ROI_Weight_MAP) ROI_Weight_MAP += this->Boundary_OffsetY;
      for (int i = 0; i < NumTotalTypeCLASS; i++) WeightsCopy[i] += this->Boundary_OffsetY;
      ...
      if (y > this->ROI_MaxY) {
    y = this->ROI_MinY;
    Boundary_ROIVectorPtr += this->Boundary_OffsetZ; 
    if (SpatialCostFunctionPtr) SpatialCostFunctionPtr += this->Boundary_OffsetZ;
    if (ROI_Weight_MAP) ROI_Weight_MAP += this->Boundary_OffsetZ;
    for (int i = 0; i < NumTotalTypeCLASS; i++) WeightsCopy[i] += this->Boundary_OffsetZ;
        result += SliceResult;
    SliceResult = 0.0;
...
inline float EMLocalRegistration::RegistrationCostFunction(float* parameters) {
  // if (this->Debug)  cout << "============ Start EMLocalRegistration::RegistrationCostFunction ===============" << endl;

  // How anoying simplex starts with index 1 and powel with index 0 ! 
  // Kilian : go through code later and change that 
  parameters --;
  ...
  if (!this->RigidFlag) {
    for (int h = 0; h < this->NumberOfParameterSets; h++) {
      ...
      if (this->TwoDFlag) {
    if ((parameters[ScaleParameterIndex +4] < EMREGISTRATION_MIN_SCALING) || (parameters[ScaleParameterIndex + 5] < EMREGISTRATION_MIN_SCALING) || 
        (parameters[ScaleParameterIndex +4] > EMREGISTRATION_MAX_SCALING) || (parameters[ScaleParameterIndex + 5] > EMREGISTRATION_MAX_SCALING)) {
          parameters ++;
      return EMLOCALREGISTRATION_MAX_PENALITY;
    }
      } else {
    if ((parameters[ScaleParameterIndex +7] < EMREGISTRATION_MIN_SCALING) || (parameters[ScaleParameterIndex + 8] < EMREGISTRATION_MIN_SCALING) 
         || (parameters[ScaleParameterIndex + 9] < EMREGISTRATION_MIN_SCALING) || (parameters[ScaleParameterIndex +7] > EMREGISTRATION_MAX_SCALING) 
         || (parameters[ScaleParameterIndex + 8] > EMREGISTRATION_MAX_SCALING) || (parameters[ScaleParameterIndex + 9] > EMREGISTRATION_MAX_SCALING)) {
          parameters ++;
      return EMLOCALREGISTRATION_MAX_PENALITY;
    }
      }
  ...
  if (this->RegistrationType != EMSEGMENT_REGISTRATION_CLASS_ONLY) {
    ...
    if (vtkSimonParameterReaderWriter::TurnParameteresIntoInverseRotationTranslation(&parameters[1], SuperClassToGlobalRotationMatrix, SuperClassToGlobalTranslationVector, 2, this->TwoDFlag,this->RigidFlag)) {
      cout << "inline float EMLocalRegistration::RegistrationCostFunction:: Could not calculate inverse of the following parameter setting :" << endl;
      EMLocalRegistration_PrintVector(parameters,1,this->NumberOfParameterPerSet);
      exit(1);
    } 
  ...
  assert(!this->ClassToAtlasRotationMatrix);
  assert(!this->ClassToAtlasTranslationVector);

  this->ClassToAtlasRotationMatrix    = new float*[this->GetNumClasses()];
  this->ClassToAtlasTranslationVector = new float*[this->GetNumClasses()];
  ...
  if (this->RegistrationType == EMSEGMENT_REGISTRATION_GLOBAL_ONLY) {
    ...
    for (int h = 0; h < this->GetNumClasses(); h++) {
      this->ClassToAtlasRotationMatrix[h]    = this->SuperClassToAtlasRotationMatrix;
      this->ClassToAtlasTranslationVector[h] = this->SuperClassToAtlasTranslationVector;
    }
    ...
    // cout << RegistrationParameters.Image_LengthX << " "  <<  RegistrationParameters.Image_LengthY << " " <<  RegistrationParameters.Image_LengthZ << endl;
    // vtkImageEMLocalSegmenter::DefineRegistrationParametersForThreadedCostFunction(RegistrationParameters.Boundary_MinX,  RegistrationParameters.Boundary_MinY, RegistrationParameters.Boundary_MinZ, RegistrationParameters.Boundary_MaxX,  RegistrationParameters.Boundary_MaxY,  RegistrationParameters.Boundary_MaxZ);
    // cout << "----------" << endl;
    // cout << "What " << RegistrationParameters.ROI_MinX <<  " " <<  RegistrationParameters.ROI_MinY <<   " " <<  RegistrationParameters.ROI_MinZ << " " <<  RegistrationParameters.ROI_MaxX <<  " " <<  RegistrationParameters.ROI_MaxY <<   " " <<  RegistrationParameters.ROI_MaxZ << endl ;
    ...
    this->ClassToAtlasRotationMatrix[h] = new float[9];
    this->ClassToAtlasTranslationVector[h] = new float[3];
    ...
    if (vtkSimonParameterReaderWriter::TurnParameteresIntoInverseRotationTranslation(&parameters[ParameterIndex], ClassToSuperClassRotationMatrix, ClassToSuperClassTranslationVector, 2, this->TwoDFlag,this->RigidFlag)) {
      cout << "inline float EMLocalRegistration::RegistrationCostFunction:: Could not calculate inverse of the following parameter setting :" << endl;
      EMLocalRegistration_PrintVector(&parameters[ParameterIndex],0,this->NumberOfParameterPerSet-1);
      exit(1);
    } 
    vtkSimonParameterReaderWriter::matmult_3x4(this->SuperClassToAtlasRotationMatrix, this->SuperClassToAtlasTranslationVector, ClassToSuperClassRotationMatrix, 
                           ClassToSuperClassTranslationVector, this->ClassToAtlasRotationMatrix[h], 
                           this->ClassToAtlasTranslationVector[h]);    
    ParameterIndex += this->NumberOfParameterPerSet; 
      } else {
  ...
  if (this->RegistrationType > EMSEGMENT_REGISTRATION_GLOBAL_ONLY) {
    ScaleParameterIndex = (this->RegistrationType > EMSEGMENT_REGISTRATION_CLASS_ONLY ?  this->NumberOfParameterPerSet : 0) + 1;
    for (int h = this->GenerateBackgroundProbability; h < this->GetNumClasses(); h++) {
    ...
    if (!this->TwoDFlag && !this->RigidFlag) { 
      ...
      GaussianPenalty = -(double(parameters[9] - 1.0) * 1000.0  * double(parameters[9] - 1.0)) * 1000.0/2.0;
     }
  }

  // So we can track the influence of the Gauss parameters
  if ( -(result + GaussianPenalty) <  this->GetMinCost()) {
    this->MinWeightAtlasCost = -result;
    this->MinGaussianCost    = -GaussianPenalty;
  ...
  if (this->RegistrationType > EMSEGMENT_REGISTRATION_GLOBAL_ONLY) {
    for (int h = this->GenerateBackgroundProbability; h < this->GetNumClasses(); h++) {
      if (this->ClassSpecificRegistrationFlag[h]) {
    delete[] this->ClassToAtlasRotationMatrix[h];
    delete[] this->ClassToAtlasTranslationVector[h];
      }
    }
  }
...
// Uses paramters as initialization of the algorithm and when completed updates the parameters with the optimal results, which have Cost associated 
void  EMLocalRegistration::StartRegistration(float* Parameters, float &Cost) {
  ...
  int NumParaTotal     =  this->NumberOfParameterSets*this->NumberOfParameterPerSet; 
  ConjugateMethods::powell_minimize(NumParaTotal, Parameters, 0.01, ((void*) this), EMLocalRegistration_RegistrationCostFunction, &Cost, NumOfFunctionEvaluations);
  ...
  if (this->RegistrationType != EMSEGMENT_REGISTRATION_CLASS_ONLY) {
    cout << "Global Parameters:    " ;EMLocalRegistration_PrintVector(Parameters,0,this->NumberOfParameterPerSet-1);
  }

  if (this->RegistrationType > EMSEGMENT_REGISTRATION_GLOBAL_ONLY) {
    for (int i = (RegistrationType > EMSEGMENT_REGISTRATION_CLASS_ONLY ? 1 : 0) ; i <  this->NumberOfParameterSets ; i++) {
        cout << "Structure Parameter " << i << ": ";  EMLocalRegistration_PrintVector(Parameters,i*this->NumberOfParameterPerSet,(i+1)*this->NumberOfParameterPerSet-1);
    }
  }

  cout << "WeightAtlas Cost: " << this->MinWeightAtlasCost << endl;
  cout << "GaussianPenality: " << this->MinGaussianCost << endl;
...
//     // ----------------------------------------------
//     Iterations ++;
//     SimplexCost[1] = RegistrationParameters.RegistrationCostFunction(SimplexParameter[1]);
...
//     }
//     SimplexCost[ParaIndex] = RegistrationParameters.RegistrationCostFunction(SimplexParameter[ParaIndex]);
//     ParaIndex ++;
//     ParaIndexMinus ++;
//       }
//     }
