...
<P> Once a statistical map of T-values has been generated, the volume may be thresholded to determine, at a given level of significance, which voxels in the brain were activated. Two methods are provided for thresholding the activation volume. With uncorrected p-values, the first approach uses the simplest method which applies the chosen threshold to every voxel in the map. This method is likely to generate some false positives, however, given the large number of voxels being tested. The second approach implements the False Discovery Rate (FDR) algorithm for thresholding the t map, which controls for the proportion of false positives relative to the number of detections. The FDR results, with and without the independence assumption, can be noticeablly different. CIND assumes the p-values are independent at different voxels while CDEP applies for any joint distribution of the p-values across voxels.
