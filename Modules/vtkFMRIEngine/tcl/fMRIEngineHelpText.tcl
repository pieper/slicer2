...
<P> In a <I>event-related</I> task paradigm, individual stimulus presentations -- or even different components of an individual stimulus presentation -- are measured at an instant in time, rather than as persisting through some finite duration. Event-related paradigms are specified by entering the number of the first volume to be included in the analysis (to eliminate dummy scans), the scan repetition time TR, a vector of stimulus presentation onsets (in multiples of TR), and a vector of stimulus intensities (currently unimplemented, so no temporal intensity changes can be modeled), In event-reltaed designs, each stimulus duration is presumably modeled by a delta function, assumed to be of duration '0' TRs. In practical terms, we model the event duration as the smallest unit of time represented in the signal (currently one second). If no vector is typed into the durations entry widget for event-related designs, the fMRIEngine will model each onset in this fashion by default.
<P><B>Mixed design </B>
...
<P> Once a statistical map of t-values has been generated, the volume may be thresholded to determine, at a given level of significance, which voxels in the brain were activated. Two methods are provided for thresholding the activation volume. The first approach is the simplest: it applies the chosen threshold to every voxel in the map and reports uncorrected p-values. With this method false positives are likely reported, given the large number of voxels being tested. The second approach implements the False Discovery Rate (FDR) algorithm (see Genovese, C.R., Lazar, N.A., Nichols, T.E., 'Thresholding of Statistical Maps in Functional Neuroimaging Using the False Discovery Rate.', NeuroImage 15:870-878, 2002) for thresholding the t map. Unlike Bonferroni and random field methods which control the chance of <I>any</I> false positives, FDR controls the expected proportion of false positives relative to the number of detections. FDR iuses a more lenient metric for false positives than traditional methods.
<P> Two options for FDR correction are provided: <I>CIND</I> and <I>CDEP</I>. The CIND option assumes the p-values are independent across voxels while the CDEP option assumes an arbitrary distribution of the p-values across voxels.
